	...
	return {
		...
		list(request: AccountMirrorCompletionListRequest = {}) {
			...
			if (
				...
				request.runtimeProfileId !== currentCompletion.runtimeProfileId
			)
				return [];
			if (
				request.status &&
				request.status !== "active" &&
				request.status !== currentCompletion.status
			)
				return [];
			if (
				request.activeOnly === true &&
				!["queued", "running", "idle_waiting", "paused"].includes(currentCompletion.status)
			)
				return [];
...
function assertLiveFollow(summary: LiveFollowHealthSummary, label: string): void {
	...
	assertEqual(
		summary.latestYield?.queuedOwnerCommand,
		expectedOwner,
		`${label} latest yield owner`,
	);
	assertEqual(
		summary.latestYield?.remainingDetailSurfaces,
		expectedRemaining,
		`${label} latest yield remaining`,
	);
}
...
function assertSchedulerDiagnosticsHints(
	...
	assertIncludes(
		hint.command,
		expectedDiagnosticsFilterFragment,
		`${label} diagnostics command filters`,
	);
...
async function main(): Promise<void> {
	...
	try {
		...
		const httpStatus = await fetchJson<{ liveFollow: LiveFollowHealthSummary }>(
			`${baseUrl}/status`,
		);
		...
		assertSchedulerDiagnosticsHints(
			cliSummary.schedulerDiagnosticsHints,
			cliText,
			"CLI scheduler diagnostics",
		);
