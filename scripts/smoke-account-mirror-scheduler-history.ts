...
import { createAccountMirrorSchedulerHistoryToolHandler } from "../src/mcp/tools/accountMirrorSchedulerHistory.js";
...
function createMemorySchedulerLedger(
	pass: AccountMirrorSchedulerPassResult,
): AccountMirrorSchedulerPassLedger {
	const entries = [pass];
...
async function main(): Promise<void> {
	...
	try {
		...
		assertEqual(
			history.latestYield?.queuedWork?.ownerCommand,
			"media-generation:chatgpt:image",
			"history latest yield owner",
		);
		assertEqual(
			history.latestYield?.remainingDetailSurfaces?.total,
			4,
			"history latest yield remaining surfaces",
		);
		assertEqual(
			history.latestYield?.resumeCursor?.nextConversationIndex,
			3,
			"history latest yield resume cursor",
		);

		const cliSummary = await readApiStatusForCli({ port: server.port });
		assertEqual(
			cliSummary.scheduler.latestYield?.queuedOwnerCommand,
			"media-generation:chatgpt:image",
			"CLI latest yield owner",
		);
		assertEqual(
			cliSummary.scheduler.latestYield?.remainingDetailSurfaces,
			4,
			"CLI latest yield remaining surfaces",
		);
		...
		assertEqual(
			structured.scheduler?.latestYield?.remainingDetailSurfaces,
			4,
			"MCP latest yield remaining surfaces",
		);
