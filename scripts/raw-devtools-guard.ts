...
export const RAW_DEVTOOLS_MUTATING_SCRIPT_ALLOWLIST = [
	"scripts/observe-chatgpt-tool-approval.ts",
...
export function consumeRawDevToolsEscapeHatch(
	...
	const flagIndex = argv.findIndex(
		(entry) => entry === RAW_DEVTOOLS_ALLOW_FLAG || entry === `${RAW_DEVTOOLS_ALLOW_FLAG}=true`,
	);
	if (flagIndex >= 0) {
...
export function requireRawDevToolsEscapeHatch(
	options: { scriptName?: string; argv?: string[]; env?: NodeJS.ProcessEnv } = {},
): void {
...
export function enforceRawDevToolsEscapeHatchForCli(
	options: { scriptName?: string; argv?: string[]; env?: NodeJS.ProcessEnv } = {},
): void {
	try {
