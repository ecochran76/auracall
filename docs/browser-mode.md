...
- `auracall profile identity-smoke --target <chatgpt|gemini|grok> [--include-negative] [--json]`: inspect the selected AuraCall runtime profile's bound service identity against the live managed browser account without submitting a prompt. By default it launches the managed browser only when no live DevTools session is registered; add `--no-launch-if-needed` for a passive check.
