...
- `oracle session <id> --open-conversation [--print-url] [--browser-path <path>]`: open the provider conversation linked to a stored session (uses the saved context, not the cache). Use `--print-url` to emit the URL only, and `--browser-path` to override the browser binary.
