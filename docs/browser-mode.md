...
- `auracall cache [--provider <chatgpt|grok>] [--refresh] [--include-history] [--history-limit <count>] [--history-since <date>]`: show cached project/conversation lists with timestamps and stale status; `--refresh` updates the cache for the active provider.
- `auracall cache export --scope <projects|conversations|conversation> --format <json|md|html|csv|zip> [--project-id <id>] [--conversation-id <id>] [--output <path>]`: export cached data to a directory or zip file.
...
- `auracall conversations [--project-id <id>] [--project-name <name>] [--conversation-name <name>] [--include-history] [--history-limit <count>] [--history-since <date>] [--filter <text>] [--refresh]`: list conversations for a provider (uses the registry or `AURACALL_BROWSER_PORT`, and spawns a manual-login Chrome session when no DevTools target is available). Use `--include-history` if you want the History dialog opened to pull older conversations; use `--history-limit` (default 200) and/or `--history-since` to scroll deeper; use `--filter` to match title/id text; use `--refresh` to force cache updates.
