...
- `oracle cache [--provider <chatgpt|grok>] [--refresh] [--include-history] [--include-project-only-conversations] [--history-limit <count>] [--history-since <date>]`: show cached project/conversation lists with timestamps and stale status; `--refresh` updates the cache for the active provider.
- `oracle cache export --scope <projects|conversations|conversation|contexts> --format <json|md|html|csv|zip> [--project-id <id>] [--conversation-id <id>] [--output <path>]`: export cached data to a directory or zip file.
- `oracle cache context list [--provider <chatgpt|grok>]`: list cached context IDs for agent/tooling use.
- `oracle cache context get <id> [--provider <chatgpt|grok>] [--output <path>]`: read one cached context (by ID or cached title) without browser scraping.
- `oracle cache context search <query> [--provider <chatgpt|grok>] [--conversation-id <id>] [--role <user|assistant|system|source>] [--limit <count>]`: keyword-search cached context chunks (messages + sources).
- `oracle cache context semantic-search <query> [--provider <chatgpt|grok>] [--model <embedding-model>] [--max-chunks <count>] [--min-score <value>] [--openai-api-key <key>] [--openai-base-url <url>]`: semantic search over cached contexts using OpenAI embeddings.
- `oracle cache sources list [--provider <chatgpt|grok>] [--conversation-id <id>] [--domain <domain>] [--source-group <group>] [--query <text>] [--limit <count>]`: list normalized source links from cache catalog (SQL-first, JSON fallback).
- `oracle cache files list [--provider <chatgpt|grok>] [--conversation-id <id>] [--project-id <id>] [--dataset <...>] [--query <text>] [--resolve-paths] [--limit <count>]`: list normalized file bindings from cache catalog (SQL-first, JSON fallback).
- `oracle cache files resolve [--provider <chatgpt|grok>] [--conversation-id <id>] [--project-id <id>] [--dataset <...>] [--query <text>] [--missing-only] [--limit <count>]`: resolve cached file pointers and classify each binding (`local_exists`, `missing_local`, `external_path`, `remote_only`, `unknown`).
- `oracle cache doctor [--provider <chatgpt|grok>] [--identity-key <key>] [--strict] [--json]`: run cache integrity checks (SQLite `quick_check`, expected table presence, and missing local file pointers).
- `oracle cache repair [--provider <chatgpt|grok>] [--identity-key <key>] [--actions <list>] [--apply --yes] [--json]`: run cache repair actions (default dry-run). Supported actions: `sync-sql`, `rebuild-index`, `prune-orphan-assets`, `mark-missing-local`.
- `oracle cache clear [--provider <chatgpt|grok>] [--identity-key <key>] [--dataset <...>] [--older-than <date>] [--include-blobs] [--yes] [--json]`: clear selected cache datasets (dry-run by default).
- `oracle cache compact [--provider <chatgpt|grok>] [--identity-key <key>] [--json]`: run SQLite `VACUUM + ANALYZE + optimize` on cache DBs.
- `oracle cache cleanup [--provider <chatgpt|grok>] [--identity-key <key>] [--older-than <date>|--days <n>] [--include-blobs] [--yes] [--json]`: cleanup stale cache files/rows, prune stale cache-index entries, and prune old backups (dry-run by default; `--days` defaults to `365` unless profile cache `cleanupDays` overrides it).
...
- `oracle conversations [--project-id <id>] [--project-name <name>] [--conversation-name <name>] [--include-history] [--history-limit <count>] [--history-since <date>] [--filter <text>] [--refresh]`: list conversations for a provider (uses the registry or `ORACLE_BROWSER_PORT`, and spawns a manual-login Chrome session when no DevTools target is available). Use `--include-history` if you want the History dialog opened to pull older conversations; use `--history-limit` (default `2000`) and/or `--history-since` to scroll deeper; use `--filter` to match title/id text; use `--refresh` to force cache updates.
