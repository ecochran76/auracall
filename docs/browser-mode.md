...
- `auracall cache [--provider <chatgpt|grok>] [--refresh] [--include-history] [--include-project-only-conversations] [--history-limit <count>] [--history-since <date>]`: show cached project/conversation lists with timestamps and stale status; `--refresh` updates the cache for the active provider.
...
- `auracall cache context list [--provider <chatgpt|grok>]`: list cached context IDs for agent/tooling use.
- `auracall cache context get <id> [--provider <chatgpt|grok>] [--output <path>]`: read one cached context (by ID or cached title) without browser scraping.
- `auracall cache context search <query> [--provider <chatgpt|grok>] [--conversation-id <id>] [--role <user|assistant|system|source>] [--limit <count>]`: keyword-search cached context chunks (messages + sources).
- `auracall cache context semantic-search <query> [--provider <chatgpt|grok>] [--model <embedding-model>] [--max-chunks <count>] [--min-score <value>] [--openai-api-key <key>] [--openai-base-url <url>]`: semantic search over cached contexts using OpenAI embeddings.
- `auracall cache sources list [--provider <chatgpt|grok>] [--conversation-id <id>] [--domain <domain>] [--source-group <group>] [--query <text>] [--limit <count>]`: list normalized source links from the cache catalog (SQL-first, JSON fallback).
- `auracall cache files list [--provider <chatgpt|grok>] [--conversation-id <id>] [--project-id <id>] [--dataset <...>] [--query <text>] [--resolve-paths] [--limit <count>]`: list normalized file bindings from the cache catalog (SQL-first, JSON fallback).
- `auracall cache files resolve [--provider <chatgpt|grok>] [--conversation-id <id>] [--project-id <id>] [--dataset <...>] [--query <text>] [--missing-only] [--limit <count>]`: resolve cached file pointers and classify each binding as `local_exists`, `missing_local`, `external_path`, `remote_only`, or `unknown`.
- `auracall cache doctor [--provider <chatgpt|grok>] [--identity-key <key>] [--strict] [--json]`: run cache integrity checks (SQLite `quick_check`, expected table presence, and missing local file pointers).
- `auracall cache repair [--provider <chatgpt|grok>] [--identity-key <key>] [--actions <list>] [--apply --yes] [--json]`: run cache repair actions (default dry-run). Supported actions include `sync-sql`, `rebuild-index`, `prune-orphan-assets`, `prune-orphan-source-links`, `prune-orphan-file-bindings`, and `mark-missing-local`.
- `auracall cache clear [--provider <chatgpt|grok>] [--identity-key <key>] [--dataset <...>] [--older-than <date>] [--include-blobs] [--yes] [--json]`: clear selected cache datasets (dry-run by default).
- `auracall cache compact [--provider <chatgpt|grok>] [--identity-key <key>] [--json]`: run SQLite `VACUUM + ANALYZE + optimize` on cache DBs.
- `auracall cache cleanup [--provider <chatgpt|grok>] [--identity-key <key>] [--older-than <date>|--days <n>] [--include-blobs] [--yes] [--json]`: clean stale cache files/rows, prune stale cache-index entries, and prune old backups (dry-run by default; `--days` defaults to `365` unless profile cache `cleanupDays` overrides it).
