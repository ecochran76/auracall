...
- `oracle conversations [--project-id <id>] [--project-name <name>] [--include-history] [--history-limit <count>] [--history-since <date>] [--filter <text>]`: list conversations for a provider (requires a running browser with DevTools; uses `ORACLE_BROWSER_PORT` or config `browser.debugPort`). Use `--include-history` if you want the History dialog opened to pull older conversations; use `--history-limit` (default 200) and/or `--history-since` to scroll deeper; use `--filter` to match title/id text.
