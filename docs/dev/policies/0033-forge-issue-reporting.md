...
- Treat issue reporting as a governed external write. A local diagnosis,
  implementation plan, authenticated CLI, or clean preflight does not by
  itself authorize creating or mutating an issue.
- Resolve an external write target from an explicit repo-local registry. Never
  infer the target from the current directory, a default remote, a similarly
  named fork, or a prior operation.
...
- Record a receipt for each mutation with authenticated actor, host, canonical
  locator and URL, action, idempotency key, timestamp, relevant
  before-and-after state, and post-write readback. A successful request without
  target readback is not a verified effect.
...
Adopt this provider-neutral module with `work-item-traceability`, then add the
