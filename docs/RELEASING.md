...
4. **Validation**
   ...
   - [ ] Lazy-live-follow operator preflight: run `pnpm run preflight:lazy-live-follow` to verify completion controls, hydration, live-follow health parity, `/ops/browser`, operator API-key issue/client auth, user-runtime install, installed MCP status tools, and direct API log-tail retrieval.
