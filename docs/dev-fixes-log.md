...
- 2026-04-15: If a browser-capable `api serve` runner still cannot project durable `serviceAccountIds`, do not invent account identities in the runner record. Preserve that limitation in the existing `eligibilityNote` so runtime inspection and operator readback stay truthful about missing account-affinity evidence.
