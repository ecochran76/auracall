...
- Behavior: creates one request through Aura-Call's shared media-generation contract and returns `object = "media_generation"` with durable artifact metadata. `wait = false` returns a running media id immediately so callers can poll `media_generation_status` or `run_status` while browser execution is active. Provider execution may still fail when the requested provider capability is unavailable or unsupported. Grok browser image requests are preflighted through `grok.media.imagine_image` on the explicit `/imagine` entrypoint and fail before prompt submission when the account is gated.
...
- Inputs: optional `provider: "chatgpt" | "gemini" | "grok"`, optional `category: "research" | "media" | "canvas" | "connector" | "skill" | "app" | "search" | "file" | "other"`, optional `runtimeProfile`, optional `includeUnavailable`, optional `diagnostics: "browser-state"`, and optional `entrypoint: "grok-imagine"`.
- Behavior: returns `object = "workbench_capability_report"` with known or discovered provider workbench capabilities, provider labels, invocation modes, surfaces, availability, stability, required inputs, output expectations, and safety flags. This is read-only discovery and does not click provider tools. `diagnostics = "browser-state"` adds bounded target/document/provider evidence and a stored PNG screenshot path for the selected provider. `entrypoint = "grok-imagine"` opens or reuses Grok `/imagine` through browser-service control-plane attribution before read-only inspection. Grok `/imagine` provider evidence can include read-only `run_state`, pending, terminal image/video, media URL, and materialization-control signals when visible.
