...
- Manual login flow: run with `--browser-manual-login --browser-keep-browser`, log into chatgpt.com in the opened Chrome, then rerun; the profile lives at `~/.oracle/browser-profile` by default (override with `ORACLE_BROWSER_PROFILE_DIR` or `browser.manualLoginProfileDir` in `~/.oracle/config.json`; legacy naming pending profile refactor). This will be renamed to `interactiveLogin`/`loginMode` in a future config update. If that automation Chrome is already running with remote debugging enabled (DevToolsActivePort present), reuse it instead of relaunching by pointing Oracle at it via `--remote-chrome <host:port>`.
