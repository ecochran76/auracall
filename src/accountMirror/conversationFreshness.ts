...
export function deriveAccountMirrorConversationFreshness(
	...
	const detailCompleteness = deriveDetailCompleteness({
		explicit: readCompletenessOverride(explicit, itemRecord, metadata, "detailCompleteness"),
		detail: input.detail,
	...
	if (
		...
		assetCounts.known > 0 &&
		assetCounts.local < assetCounts.known
	) {
		assetCounts = {
...
function deriveRouteabilityState(input: {
	...
	if (/identity[_ -]?mismatch|wrong managed browser profile/i.test(reason))
		return "identity_mismatch";
	if (
		/google\.com\/sorry|captcha|recaptcha|human.verification|anti-bot|provider[-_ ]guard/i.test(
			reason,
		)
	) {
		return "guarded";
	...
	if (
		input.providerGuard?.state === "manual_clear_required" ||
		input.providerGuard?.state === "cooldown"
	) {
		return "guarded";
	}
	return "unknown";
...
function readCompletenessOverride(
	...
	return normalizeCompleteness(
		readString(explicit, [field]) ?? readString(item, [field]) ?? readString(metadata, [field]),
	);
}

function countAssetAvailability(
	assets: unknown[],
): AccountMirrorConversationFreshness["assetCounts"] {
	let known = 0;
	let local = 0;
	let missingLocal = 0;
...
function hasKnownAssetEvidence(asset: Record<string, unknown>): boolean {
	return Boolean(
		readString(asset, [
			...
			readString(readRecord(asset, "metadata"), [
				"id",
				"artifactId",
				"fileId",
				"mediaId",
...
function fingerprintConversation(
	...
	const source = {
		...
		latestTurnId:
			readString(record, ["latestTurnId", "lastMessageId"]) ??
			readString(metadata, ["latestTurnId", "lastMessageId"]),
		messageCount:
			detail?.messageCount ?? readNumber(record, ["messageCount", "messagesCount", "turnCount"]),
		fileCount:
			detail?.fileCount ?? readNumber(record, ["fileCount", "filesCount", "cachedFileCount"]),
		artifactCount:
			detail?.artifactCount ??
			readNumber(record, ["artifactCount", "artifactsCount", "cachedArtifactCount"]),
		sourceCount:
			detail?.sourceCount ??
			readNumber(record, ["sourceCount", "sourcesCount", "cachedSourceCount"]),
		assetCounts,
	};
...
function isFingerprintChanged(
	...
	const previous = readString(explicit, [
		"previousConversationFingerprint",
		"priorConversationFingerprint",
	]);
...
function normalizeFreshnessState(
	value: string | null,
): AccountMirrorConversationFreshnessState | null {
	switch (value) {
		case "fresh":
		case "stale":
		case "partial":
...
function normalizeRouteabilityState(
	value: string | null,
): AccountMirrorConversationRouteabilityState | null {
	switch (value) {
		case "routeable":
