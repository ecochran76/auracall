...
export interface AccountMirrorPersistence {
  ...
  updateConversationEvidence?(input: AccountMirrorConversationContextRequest & {
  ...
  readConversationContext(input: AccountMirrorConversationContextRequest): Promise<ConversationContext | null>;
  ...
  return {
    ...
    async updateConversationEvidence(request) {
      ...
      if (!updated) {
        ...
        next.push(mergeConversationEvidence({
    ...
    async readState(request) {
      ...
      const snapshotCompletedAtMs = Date.parse(snapshot.refresh.completedAt || snapshot.collectedAt);
      const shouldApplyStatusRecord = statusRecord
        ? !Number.isFinite(snapshotCompletedAtMs) || Date.parse(statusRecord.updatedAt) >= snapshotCompletedAtMs
        : false;
      if (
...
function resolveAccountMirrorStatusDir(config: Record<string, unknown> | null | undefined): string {
  return path.join(readCacheRoot(config) ?? path.join(getAuracallHomeDir(), 'cache'), 'account-mirror', 'status');
}
...
function resolveStatusRecordPath(
  ...
  const digest = createHash('sha256')
    ...
    .digest('hex');
  return path.join(rootDir, `${key.provider}-${encodeURIComponent(key.runtimeProfileId)}-${digest.slice(0, 16)}.json`);
}
...
function parsePersistentStatusRecord(
  ...
  if (
    ...
    (typeof value.browserProfileId === 'string' ? value.browserProfileId : null) !== key.browserProfileId ||
    normalizeIdentityKey(String(value.boundIdentityKey ?? '')) !== normalizeIdentityKey(key.boundIdentityKey ?? '')
  ) {
    return null;
  }
...
function mergeStatusState(
  ...
  for (const [key, value] of Object.entries(overlay) as Array<[keyof AccountMirrorStatusState, unknown]>) {
