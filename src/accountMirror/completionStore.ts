...
import type {
	AccountMirrorLiveFollowCycleLedger,
...
export interface AccountMirrorCompletionStore {
	...
	): Promise<AccountMirrorCompletionStoredRecord>;
...
function resolveAccountMirrorCompletionsDir(
	...
	const cacheRoot =
		readNestedString(config, ["browser", "cache", "rootDir"]) ??
...
async function readStoredRecord(
	rootDir: string,
	id: string,
): Promise<AccountMirrorCompletionStoredRecord | null> {
	return readStoredRecordFile(resolveCompletionRecordPath(rootDir, id));
}

async function readStoredRecordFile(
	recordPath: string,
): Promise<AccountMirrorCompletionStoredRecord | null> {
	try {
...
function parseOperation(value: unknown): AccountMirrorCompletionOperation {
	...
	return {
		...
		lastRefresh: isRecord(value.lastRefresh)
			? (value.lastRefresh as unknown as AccountMirrorCompletionOperation["lastRefresh"])
			: null,
		materializationPolicy: readMaterializationPolicy(
			value.materializationPolicy,
			readSweepMode(value.sweepMode),
		),
		...
		materializationRefreshSnapshot:
			readBoolean(value.materializationRefreshSnapshot) ??
			readSweepMode(value.sweepMode) === "full_sweep",
...
function parseLifecycleEvents(
	value: unknown,
): NonNullable<AccountMirrorCompletionOperation["lifecycleEvents"]> {
	...
	return value
		...
		.filter(
			(entry): entry is NonNullable<AccountMirrorCompletionOperation["lifecycleEvents"]>[number] =>
				entry !== null,
		)
		.slice(-20);
}
...
function parseMaterializationCursor(
	value: unknown,
): AccountMirrorCompletionMaterializationCursor | null {
	...
	return {
		jobId,
		jobStatus:
			typeof value.jobStatus === "string" && value.jobStatus.trim()
				? value.jobStatus.trim()
				: "unknown",
		...
		request: {
			...
			runtimeProfile:
				typeof value.request.runtimeProfile === "string" && value.request.runtimeProfile.trim()
...
function parseAccountLibraryCursor(
	value: unknown,
): AccountMirrorCompletionOperation["accountLibraryCursor"] {
	if (!isRecord(value)) return null;
		...
		? {
				...
				runtimeProfile:
					typeof value.request.runtimeProfile === "string" && value.request.runtimeProfile.trim()
						...
						: "default",
				browserProfile:
					typeof value.request.browserProfile === "string" && value.request.browserProfile.trim()
						? value.request.browserProfile.trim()
						: null,
				boundIdentityKey:
					typeof value.request.boundIdentityKey === "string" &&
					value.request.boundIdentityKey.trim()
						? value.request.boundIdentityKey.trim()
						: null,
				reconcile: true as const,
				assetSource:
					value.request.assetSource === "account-library" ? ("account-library" as const) : null,
				refreshSnapshot: readBoolean(value.request.refreshSnapshot) ?? false,
				assetKinds: readMaterializationAssetKinds(value.request.assetKinds),
				maxItems: readMaterializationMaxItems(value.request.maxItems),
	...
	return {
		...
		jobStatus:
			typeof value.jobStatus === "string" && value.jobStatus.trim() ? value.jobStatus.trim() : null,
		reused: readBoolean(value.reused) ?? false,
		requestedAt: normalizeIsoString(value.requestedAt) ?? new Date(0).toISOString(),
		passCount,
		status,
...
function parseMaterializationOutcome(
	value: unknown,
): AccountMirrorCompletionOperation["materializationOutcome"] {
	if (!isRecord(value)) return null;
...
function readLifecycleEventType(
	value: unknown,
): NonNullable<AccountMirrorCompletionOperation["lifecycleEvents"]>[number]["type"] | null {
	if (
		value === "started" ||
...
function parseLiveFollowCycleLedger(value: unknown): AccountMirrorLiveFollowCycleLedger | null {
	if (!isRecord(value)) return null;
	const cycleId =
		typeof value.cycleId === "string" && value.cycleId.trim() ? value.cycleId.trim() : "";
	if (!cycleId) return null;
	const currentPhase = readLiveFollowCyclePhase(value.currentPhase);
	const nextPhase = readLiveFollowCyclePhase(value.nextPhase) ?? currentPhase;
	if (!currentPhase || !nextPhase) return null;
	const startedAt = normalizeIsoString(value.startedAt) ?? new Date(0).toISOString();
	const updatedAt = normalizeIsoString(value.updatedAt) ?? startedAt;
	return {
		cycleId,
		startedAt,
		updatedAt,
		currentPhase,
		nextPhase,
		decisionReason: typeof value.decisionReason === "string" ? value.decisionReason : "",
		passCount: Math.max(0, Math.floor(readNumber(value.passCount) ?? 0)),
		phases: parseLiveFollowCyclePhaseEntries(value.phases),
	};
}

function parseLiveFollowCyclePhaseEntries(
	value: unknown,
): AccountMirrorLiveFollowCycleLedger["phases"] {
	if (!Array.isArray(value)) return [];
	return value
		.map((entry) => {
			if (!isRecord(entry)) return null;
			const phase = readLiveFollowCyclePhase(entry.phase);
			const status = readLiveFollowCyclePhaseStatus(entry.status);
			const updatedAt = normalizeIsoString(entry.updatedAt);
			if (!phase || !status || !updatedAt) return null;
			return {
				phase,
				status,
				reason: typeof entry.reason === "string" ? entry.reason : "",
				updatedAt,
				passCount: Math.max(0, Math.floor(readNumber(entry.passCount) ?? 0)),
			} satisfies AccountMirrorLiveFollowCycleLedger["phases"][number];
		})
		.filter(
			(entry): entry is AccountMirrorLiveFollowCycleLedger["phases"][number] => entry !== null,
		)
		.slice(-12);
}
