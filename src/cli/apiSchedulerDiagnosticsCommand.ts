...
export function formatApiSchedulerDiagnosticsCliSummary(
	summary: ApiSchedulerDiagnosticsCliSummary,
): string {
	...
	const browserMutations = isRecord(diagnostics.browserMutations)
		? diagnostics.browserMutations
		: null;
	const lines = [
	...
	if (browserMutations) {
		...
		const duplicateSameRouteAttempts = isRecord(browserMutations.duplicateSameRouteAttempts)
			? browserMutations.duplicateSameRouteAttempts
			: null;
		if (duplicateSameRouteAttempts) {
			lines.push(
				`Duplicate same-route attempts: ${readNumber(duplicateSameRouteAttempts.total) ?? "unknown"}`,
			);
		}
	}
	return lines.join("\n");
