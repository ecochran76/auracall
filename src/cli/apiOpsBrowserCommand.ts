...
export interface ApiOpsBrowserDashboardSummary {
  ...
  hasRecentServiceEventFilters: boolean;
  hasRecentServiceEventSchedulerDetail: boolean;
  hasRecentServiceEventPersistence: boolean;
  hasPreflightStatusPanel: boolean;
  hasPreflightRunControl: boolean;
  hasPreflightRunHistoryPanel: boolean;
  hasPreflightStepProgress: boolean;
  hasPreflightRunLogControl: boolean;
  hasBrowserProcessPanel: boolean;
  usesBrowserProcessesPath: boolean;
  hasServiceDiscoveryPanel: boolean;
  hasBackgroundDrainControls: boolean;
  hasMirrorSchedulerControls: boolean;
  hasMirrorSchedulerExplanation: boolean;
  hasMirrorSchedulerWaitTable: boolean;
  hasMirrorSchedulerWaitActions: boolean;
  hasMirrorSchedulerCompletionDetail: boolean;
  hasMirrorSchedulerDiagnosticsBundle: boolean;
  hasMirrorSchedulerForegroundWork: boolean;
  ...
  hasRunOnePassCompletionAction: boolean;
  ...
  hasCatalogRowReconciliationActions: boolean;
...
function assertDashboardContract(summary: ApiOpsBrowserDashboardSummary): void {
  const checks: Array<[boolean, string]> = [
    ...
    [summary.hasRecentServiceEventFilters, 'Expected /ops/browser recent service events to filter by source.'],
    [summary.hasRecentServiceEventSchedulerDetail, 'Expected /ops/browser recent service events to inspect scheduler detail.'],
    [summary.hasRecentServiceEventPersistence, 'Expected /ops/browser recent service events to persist filter and selected detail.'],
    ...
    [summary.hasPreflightStepProgress, 'Expected /ops/browser to expose preflight per-step progress.'],
    [summary.hasPreflightRunLogControl, 'Expected /ops/browser to open bounded preflight run logs.'],
    [summary.hasBrowserProcessPanel, 'Expected /ops/browser to include browser process diagnostics.'],
    [summary.usesBrowserProcessesPath, 'Expected /ops/browser browser process diagnostics to read /v1/browser/processes.'],
    ...
    [summary.hasMirrorSchedulerExplanation, 'Expected /ops/browser to explain mirror scheduler waiting state.'],
    [summary.hasMirrorSchedulerForegroundWork, 'Expected /ops/browser to expose foreground-work scheduler pressure.'],
    [summary.hasMirrorSchedulerWaitTable, 'Expected /ops/browser to include per-target mirror scheduler wait rows.'],
    [summary.hasMirrorSchedulerWaitActions, 'Expected /ops/browser scheduler wait rows to include read-only operator actions.'],
    [summary.hasMirrorSchedulerCompletionDetail, 'Expected /ops/browser scheduler wait row inspection to render local completion detail.'],
    [summary.hasMirrorSchedulerDiagnosticsBundle, 'Expected /ops/browser scheduler wait rows to copy an operator diagnostics bundle.'],
    ...
    [summary.hasRunOnePassCompletionAction, 'Expected /ops/browser target rows to include run-one-pass completion controls.'],
    ...
    [summary.hasCatalogRowReconciliationActions, 'Expected /ops/browser catalog conversation rows to queue immediate reconciliation.'],
...
function summarizeDashboardHtml(html: string): ApiOpsBrowserDashboardSummary {
  return {
    ...
    hasRecentServiceEventFilters: html.includes('recentServiceEventFilter')
      && html.includes('filterRecentServiceEvents')
      && html.includes('recentServiceEventVisibleCount')
      && html.includes('value="scheduler"'),
    hasRecentServiceEventSchedulerDetail: html.includes('recentServiceEventDetail')
      && html.includes('showRecentServiceEventDetail')
      && html.includes('compactSchedulerEventDetail')
      && html.includes('Inspect Scheduler'),
    hasRecentServiceEventPersistence: html.includes('RECENT_SERVICE_EVENT_FILTER_STORAGE_KEY')
      && html.includes('RECENT_SERVICE_EVENT_SELECTED_STORAGE_KEY')
      && html.includes('restoreRecentServiceEventFilter')
      && html.includes('restoreRecentServiceEventSelection')
      && html.includes('persistRecentServiceEventSelection')
      && html.includes('service-event-row-selected'),
    hasPreflightStatusPanel: html.includes('Preflight Completed')
      && html.includes('renderPreflightStatus')
      && html.includes('lazyLiveFollow'),
    hasPreflightRunControl: html.includes('preflightControls')
      && html.includes('runLazyLiveFollowPreflight')
      && html.includes("preflight: { action: 'run', name: 'lazy-live-follow' }"),
    hasPreflightRunHistoryPanel: html.includes('preflightRunHistory')
      && html.includes('lazyLiveFollowRunHistory')
      && html.includes('renderPreflightRunHistory'),
    hasPreflightStepProgress: html.includes('preflightStepProgress')
      && html.includes('renderPreflightStepProgress')
      && html.includes('data-preflight-step-status')
      && html.includes('No preflight step progress recorded.'),
    hasPreflightRunLogControl: html.includes('preflightRunLog')
      && html.includes('loadPreflightRunLog')
      && html.includes('/v1/preflight/lazy-live-follow/runs/'),
    hasBrowserProcessPanel: html.includes('browserProcessPanel')
      && html.includes('Browser Processes')
      && html.includes('renderBrowserProcesses')
      && html.includes('launchCommandHasBlankArg')
      && html.includes('openBlankPageCount')
      && html.includes('Separates Chrome launch arguments from the actual DevTools page targets'),
    usesBrowserProcessesPath: html.includes('/v1/browser/processes'),
    ...
    hasMirrorSchedulerExplanation: html.includes('mirrorSchedulerExplanation')
      && html.includes('buildMirrorSchedulerExplanation')
      && html.includes('latestMirrorSchedulerPass')
      && html.includes('mirrorSchedulerNextRetry')
      && html.includes('mirrorSchedulerRoutineEligible'),
    hasMirrorSchedulerWaitTable: html.includes('mirrorSchedulerWaitTable')
      && html.includes('renderMirrorSchedulerWaitTable')
      && html.includes('classifyMirrorSchedulerTargetWait')
      && html.includes('data-mirror-scheduler-wait-row')
      && html.includes('routine cadence')
      && html.includes('retry delay'),
    hasMirrorSchedulerWaitActions: html.includes('renderMirrorSchedulerWaitRowActions')
      && html.includes('buildMirrorSchedulerAccountMirrorPath')
      && html.includes('Inspect completion')
      && html.includes('Open cache')
      && html.includes('data-mirror-scheduler-cache-link'),
    hasMirrorSchedulerCompletionDetail: html.includes('mirrorSchedulerCompletionDetail')
      && html.includes('setMirrorSchedulerCompletionDetail')
      && html.includes('Select a scheduler wait row completion')
    ...
    hasMirrorSchedulerDiagnosticsBundle: html.includes('openMirrorSchedulerDiagnostics')
      ...
      && html.includes('renderMirrorSchedulerDiagnosticsCommandHint')
      ...
      && html.includes('data-mirror-scheduler-diagnostics-command-list')
      && html.includes('data-mirror-scheduler-diagnostics-command-row')
    ...
    hasMirrorSchedulerForegroundWork: html.includes('mirrorSchedulerForegroundWork')
      && html.includes('data-mirror-scheduler-foreground-work')
      && html.includes('formatMirrorSchedulerForegroundWorkText')
      && html.includes('scheduler.foregroundWork'),
    ...
    hasRunOnePassCompletionAction: hasInlineCompletionAction(html, 'run_one_pass', 'Run 1'),
    ...
    hasCatalogRowPreviewActions: html.includes('renderCatalogRowActions')
      ...
      && html.includes('link-button'),
    hasCatalogRowReconciliationActions: html.includes('canReconcileMirrorCatalogRow')
      && html.includes('reconcileMirrorCatalogRow')
      && html.includes('/v1/account-mirrors/materializations')
      && html.includes("catalogKind: 'conversations'")
      && html.includes("refreshSnapshot: true")
      && html.includes('Reconcile'),
