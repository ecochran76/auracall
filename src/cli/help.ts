...
function renderHelpBanner(version: string, colors: HelpColors): string {
  const subtitle = 'Prompt + files required — API models and semantic ChatGPT browser selectors for tough questions with code/file context.';
...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet('•')} Non-preview runs spawn detached sessions (especially Pro API runs). If the CLI times out, do not re-run — reattach with ${colors.accent('auracall session <slug>')} to resume/inspect the existing run.`,
