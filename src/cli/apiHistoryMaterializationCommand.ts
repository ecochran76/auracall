...
export async function createApiHistoryMaterializationJobForCli(
	...
	try {
		const response = await fetchWithLocalApiAuth(
			new URL(`http://${host}:${port}/v1/account-mirrors/materializations`),
			{
				method: "POST",
...
function formatSchedulerDiagnostics(value: unknown): string | null {
	...
	const parts = [`state=${state ?? "unknown"}`, `dispatch=${dispatchState ?? "unknown"}`];
	...
	if (queuedToStartLatencyMs !== null)
		parts.push(`queuedToStartLatencyMs=${queuedToStartLatencyMs}`);
...
function formatSource(value: unknown): string {
	...
	if (type === "conversation")
		return `${type}:${readString(source.provider) ?? "unknown"}/${readString(source.conversationId) ?? "unknown"}`;
	...
	if (type === "project_sources")
		return `${type}:${readString(source.provider) ?? "unknown"}/${readString(source.projectId) ?? "unknown"}`;
	if (type === "reconciliation") return `${type}:${readString(source.provider) ?? "all"}`;
	if (type === "account_library_reconciliation")
		return `${type}:${readString(source.provider) ?? "all"}`;
	return type;
}
