...
### Fixed
- Browser/ChatGPT: rebase browser-model discovery/selection onto ChatGPT's current semantic picker (`Instant`, `Thinking`, `Pro`) instead of stale versioned `GPT-5.2 ...` labels, make browser-mode default to `Instant`, discover the active model from the checked open-menu row instead of the generic `ChatGPT` button caption, and recognize ChatGPT's new trailing-slot selected-state marker so non-Pro model switching stays reliable on the managed WSL browser session.
- Browser/ChatGPT: support ChatGPT's current composer add-on surface in browser mode via `--browser-composer-tool <tool>`, treat `Standard` / `Extended` as the live thinking-depth labels while keeping `light` / `heavy` as legacy aliases, dismiss stale open menus before model/thinking/tool selection, and harden shared menu helpers for `menuitemradio` / `option` roles so managed WSL ChatGPT runs can reliably apply add-ons like `Web search` without confusing the Thinking menu for the `Add files and more` surface.
- Browser/ChatGPT: lock `--browser-composer-tool` onto the current live add-on catalog by mapping the observed top-level and `More` submenu labels (`Deep research`, `Create image`, `Study and learn`, `Agent mode`, `Canvas`, `GitHub`, `Gmail`, `Google Calendar`, `Google Drive`, `Intuit QuickBooks`, `Quizzes`, etc.) through stable shorthand aliases while keeping true file upload on the normal attachment path instead of treating it as a composer tool.
...
- Browser/ChatGPT: finish ChatGPT project instructions CRUD on the managed WSL Chrome path via `auracall projects instructions get|set --target chatgpt`, verify writes by reopening the same project settings sheet and confirming the persisted `Instructions` textarea value, reuse that same verification for create-with-instructions, and accept delete success when ChatGPT removes the project but leaves the selected tab parked on the stale project route until the next navigation.
- Browser/ChatGPT: add root conversation CRUD on the managed WSL Chrome path via `auracall conversations --target chatgpt`, `auracall conversations context get`, `auracall rename`, and `auracall delete`, including canonical root/project conversation URL resolution, sidebar conversation scraping, row-menu rename/delete, turn-DOM retry/reload handling for context reads, and refreshed-list verification so disposable ChatGPT conversations can now be listed, read, renamed, and deleted cleanly without Pro.
- Browser/ChatGPT: correct the current root conversation action-surface split by moving rename back onto the ChatGPT home/sidebar row menu (the open-conversation header menu is not a real rename surface on today's DOM), teach shared inline-rename submission to use a real CDP Enter key when needed, and move root delete onto the same list-first row strategy with header delete kept only as fallback. The remaining open ChatGPT conversation nuance is now fresh-id catalog resolution for standalone `delete <conversationId>` calls, not rename-menu drift.
- Browser/ChatGPT: add provider-native root conversation-id passthrough so standalone `auracall delete <conversationId> --target chatgpt` no longer depends on the refreshed conversation catalog, and teach the ChatGPT acceptance runner to wait once across a known guard cooldown during the final delete/project-remove cleanup instead of reporting a false end-of-run failure.
- Browser/ChatGPT: add the first real conversation-file read surface on the managed WSL Chrome path via `auracall conversations files list <conversationId> --target chatgpt` plus `files[]` in `auracall conversations context get ... --json-only`, scrape the current sent user-turn upload tiles as the authoritative file-read surface, model file identity synthetically from conversation id + turn/message id + tile index + filename because the live tile DOM does not currently expose a stronger native file id, and harden conversation delete verification by rechecking from the authoritative list surface before treating stale current-page anchors as a real post-delete failure.
- Browser/ChatGPT: add the first `auracall conversations artifacts fetch <conversationId> --target chatgpt` materialization path, storing generated image artifacts as fetched `.png` files and inline `ada_visualizations` table artifacts as CSVs under `conversation-attachments/<conversationId>/files/...`, while deliberately leaving markdown-only `sandbox:/...` downloads metadata-only until a stable binary resolver exists.
- Browser/ChatGPT: finish the markdown-backed workbook/materialization gap for `sandbox:/...xlsx` spreadsheet artifacts by falling back to the embedded spreadsheet card's header download button when no filename-matching behavior button exists, fetch the resulting signed `backend-api/estuary/content?id=file_...` URL directly, and write a sidecar `conversation-attachments/<conversationId>/artifact-fetch-manifest.json` with per-artifact `materialized|skipped|error` results without changing the existing attachment manifest schema.
- Browser/ChatGPT: replace the old flat write guard with a weighted post-commit pacing model. ChatGPT writes now persist weighted mutation records, lighter actions like rename/instructions count less than create/upload/browser-send, every successful write opens a post-commit quiet period before the next refresh-heavy or mutating step, and both llmservice CRUD and browser-mode sends enforce that adaptive quiet period plus the weighted rolling budget before falling back to the existing visible rate-limit cooldown path.
- Browser/ChatGPT: add provider-local visible rate-limit dialog recovery for `auracall conversations context get ... --target chatgpt` and `auracall conversations artifacts fetch ... --target chatgpt`. When ChatGPT surfaces a visible `Too many requests` / `...too quickly` modal during those read/materialization flows, Aura-Call now dismisses it, pauses briefly, retries once, and only then escalates to the persisted cross-process cooldown guard. Serialized full-context ingestion is also re-proven on representative image, DOCX + canvas, and workbook chats.
- Browser/ChatGPT: enrich `auracall conversations context get <conversationId> --target chatgpt --json-only` with assistant-side `sources[]` plus `artifacts[]`, including file-backed citation metadata from ChatGPT's backend conversation payload, downloadable `sandbox:/...` outputs like `updated skill.zip` / `combined JSON extraction` / `combined BibTeX extraction`, generated `image_asset_pointer` outputs normalized into first-class `image` artifacts with `sediment://...` URIs plus size/dimension and DALL-E generation metadata, and canvas/textdoc artifacts with captured `textdocId`, title, and content text. The robust capture path now falls back to CDP network payload capture on a reload because direct in-page fetches can return JSON `conversation_not_found` responses even on correctly hydrated ChatGPT pages.
- Browser/ChatGPT: extend that same context/artifact path to ChatGPT table outputs by normalizing `metadata.ada_visualizations` entries with `type = table` into first-class `spreadsheet` artifacts backed by durable `chatgpt://file/<file_id>` URIs, so CSV/table chats no longer disappear from `artifacts[]` just because they are not markdown downloads or canvas blocks.
- Browser/ChatGPT: classify spreadsheet-like markdown downloads (`.csv`, `.tsv`, `.xls`, `.xlsx`, `.ods`) as `spreadsheet` artifacts instead of generic `download`, so chats that expose workbook outputs as plain `sandbox:/...` links still land in the right artifact family.
- Browser/ChatGPT: tighten root-conversation rename verification around ChatGPT's real completion behavior: after `Enter`, the same conversation id now has to bubble to the top of the root conversation list with the new title before Aura-Call calls rename successful. The guarded ChatGPT acceptance runner now uses that same top-row reorder signal, and project-chat verification explicitly keeps the project page conversation list as authoritative instead of the abbreviated sidebar subset.
- Browser/ChatGPT: make project-scoped conversation CRUD explicitly trust the project page `Chats` panel instead of generic `/c/...` scraping. Project-scoped list/rename/delete now prefer the visible project-page `tabpanel` rows over any selected-project sidebar subset, project-chat rename verification now prefers the row-menu label plus one fresh list reload before failing, and the ChatGPT acceptance runner now includes a disposable project-conversation CRUD slice alongside the existing root-conversation checks.
- Browser/ChatGPT: persist a profile-scoped live-UI rate-limit guard for ChatGPT under `~/.auracall/cache/providers/chatgpt/__runtime__/rate-limit-<profile>.json`, automatically space ChatGPT mutating llmservice operations apart, enforce a rolling per-profile write budget before another ChatGPT mutation/browser send is attempted, and make both later live ChatGPT CRUD calls and browser-mode prompt runs fail fast on the same cooldown after the UI surfaces `Too many requests` / `...too quickly` instead of letting fresh CLI processes keep hammering the same account.
- Browser/ChatGPT: finish the guarded WSL ChatGPT acceptance bar by giving `scripts/chatgpt-acceptance.ts` longer mutation timeouts plus write-budget-aware retry handling, trusting the real `Delete chat?` dialog + `delete-conversation-confirm-button` over stale page-title text during root delete, scoping `Delete project` to the tagged project-settings dialog so overlapping `Too many requests` dialogs no longer poison final project cleanup, aborting long persisted ChatGPT cooldown waits instead of sleeping for minutes and resuming later, and adding resumable `--phase ...` modes so throttled accounts can validate project/root/tool slices across multiple passes instead of one giant write burst.
- Browser/ChatGPT: inspect and persist existing-conversation composer tool state instead of inferring it from a successful send, by teaching browser-mode to read the current selected add-on from the live chip/menu surface, store it in `browser.runtime.composerTool`, and persist the final normalized `conversationId` in session runtime metadata so prompt-matched acceptance/debug flows can prove which add-on was really active on an existing conversation.
- Browser/ChatGPT: finish the phased non-Pro WSL acceptance sweep after two last-mile fixes: project-page conversation lists now prefer the row action label over noisy anchor text when deriving project-chat titles, and existing-conversation browser runs now reject reused assistant turns when ChatGPT shows a visible `Too many requests` modal instead of incorrectly returning the previous assistant answer as false success. The default ChatGPT rolling write budget is also stricter now (`3` writes per `2` minutes) so chained live CRUD/browser phases back off earlier on this account.
...
- Browser-service/ChatGPT: add shared visible-menu inventory plus expected-item-aware menu-family selection to `openMenu(...)` / `waitForMenuOpen(...)`, return specific tagged selectors for the chosen visible menu, and use that shared path for ChatGPT project-create memory mode so `Project settings` no longer depends on the first visible `[role="menu"]`.
- Browser-service/ChatGPT: keep visible-menu selectors stable across repeated inventory reads, promote `openSubmenu(...)` / `selectNestedMenuPath(...)` into the shared browser-service layer, and use those stable menu handles for ChatGPT composer add-ons so `--browser-composer-tool canvas` and other `More` submenu tools survive top-menu inspection plus reopen-to-verify flows on the managed WSL browser session.
- Browser-service/ChatGPT: add package-owned select-and-reopen verification via `inspectNestedMenuPathSelection(...)` and `selectAndVerifyNestedMenuPathOption(...)`, then move ChatGPT composer add-on verification onto that shared path so submenu tools like `Canvas` no longer need provider-local reopen/selected-state glue.
- Browser-service/ChatGPT: add package-owned overlay inventory plus blocking-surface recovery. Browser-service now exposes `collectVisibleOverlayInventory(...)`, `dismissOverlayRoot(...)`, and `withBlockingSurfaceRecovery(...)`, and ChatGPT context/artifact rate-limit modal handling now uses those shared helpers instead of a provider-local detect/dismiss/retry loop.
