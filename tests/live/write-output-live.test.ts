...
const ENABLE_LIVE = process.env.AURACALL_LIVE_TEST === '1' && process.env.OPENAI_API_KEY && !isOpenRouterBase;

if (!ENABLE_LIVE) {
  describe.skip('write-output live e2e', () => {
    test('Set AURACALL_LIVE_TEST=1 with a real OPENAI_API_KEY (api.openai.com) to run this suite.', () => {});
  });
} else {
