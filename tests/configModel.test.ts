...
describe('config model helpers', () => {
  ...
  it('keeps agent defaults execution-inert for runtime selection and default service resolution', () => {
    const config = {
      defaultRuntimeProfile: 'default',
      browserProfiles: {
        default: { chromePath: '/chrome/default' },
      ...
      agents: {
        analyst: {
          ...
          defaults: {
            ...
            projectName: 'from-agent-defaults',
            browserProfile: 'consulting',
            runtimeProfile: 'work',
          },
        },
    ...
    expect(analyzeConfigModelBridgeHealth(config, { explicitProfileName: 'default' })).toEqual({
      ...
      issues: [
        expect.objectContaining({
          ...
          severity: 'info',
          message:
            'Top-level browser config still defines transitional compatibility-alias service/project-scoped defaults (browser.modelStrategy, browser.thinkingTime, browser.projectName, browser.projectId, browser.conversationName, browser.conversationId); prefer services.<service> or runtimeProfiles.<name>.services.<service> for these defaults and treat the root browser layer as compatibility input rather than the primary authoring surface.',
        }),
      ],
    });
  });
  ...
  it('warns when manualLoginProfileDir is set without active manualLogin', () => {
    const config = {
      defaultRuntimeProfile: 'default',
      browserProfiles: {
        default: {
    ...
    expect(analyzeConfigModelBridgeHealth(config, { explicitProfileName: 'default' })).toEqual({
      ...
      issues: [
        ...
        expect.objectContaining({
          ...
          auracallRuntimeProfile: 'default',
          message:
            'AuraCall runtime profile "default" still defines manualLoginProfileDir without active manualLogin (browser.manualLoginProfileDir, services.grok.manualLoginProfileDir); these paths are inert until manualLogin is explicitly true for the same scope.',
        }),
      ],
    });
  });
