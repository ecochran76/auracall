...
<section id="recentServiceEventsPanel"><h2>Recent Service Events</h2><select id="recentServiceEventFilter" onchange="filterRecentServiceEvents()"><option value="scheduler">Scheduler</option></select><span id="recentServiceEventVisibleCount">1 shown</span><div id="recentServiceEvents"><table id="recentServiceEventsTable"><tr class="service-event-row-selected" data-recent-service-event-source="api"><td><button onclick="loadApiLogTail()">Open API Log</button><button onclick="loadPreflightRunLog('run_1')">Open Preflight Log</button><button onclick="showRecentServiceEventDetail('0')">Inspect Scheduler</button></td></tr></table></div><pre id="recentServiceEventDetail"></pre></section>
...
<div id="mirrorTargetTable"><table id="mirrorTargetAccounts"><tr data-mirror-target-row="true" data-mirror-target-completeness="in_progress" data-mirror-target-attention="true"><td data-mirror-target-attention-reason="true">failure backoff after collector timed out</td></tr></table></div>
...
<script>
  ...
  function renderRecentServiceEventRow(event) { return '<tr data-recent-service-event-source="scheduler"><td>Open API Log</td><td>Open Preflight Log</td><td>Inspect Scheduler</td></tr>'; }
  ...
  function showRecentServiceEventDetail(index) { return compactSchedulerEventDetail(index); }
  function restoreRecentServiceEventFilter() {}
  function restoreRecentServiceEventSelection() {}
  function persistRecentServiceEventSelection() {}
  function compactSchedulerEventDetail(entry) { return entry; }
  ...
  function formatMirrorTargetAttentionReason(target) { return target.latestCompletionError ? 'failure backoff after ' + target.latestCompletionError : 'backfill remains incomplete or no attention needed'; }
  ...
  function collectVisibleAgentsRecentMirrorLinks() { return ['/account-mirror?item=conv_1']; }
  async function copyVisibleAgentsRecentMirrorLinks() { await navigator.clipboard.writeText(collectVisibleAgentsRecentMirrorLinks().join('\\n')); return 'Copied 1 visible mirror link(s).'; }
  function renderAgentsRecentMirrorSummary(summary) { return summary.firstAccountMirrorPath + summary.firstCatalogItemPath + summary.conversations + renderAgentsRecentMirrorCacheBadge(summary) + renderAgentsRecentMirrorCacheSummary() + '<button data-account-mirror-path="/account-mirror?item=conv_1" onclick="openAgentsRecentMirrorSummary(this)">1 cached conversation</button>'; }
  ...
  function renderAgentsRecentMirrorCacheSummary() { return '<div data-agents-recent-mirror-cache-summary="pending">cache summary pending</div>'; }
  ...
  function summarizeAgentsRecentMirrorCacheRows() { return formatAgentsRecentMirrorCacheSummary({ transcript: 1, assets: 1, metadata: 1, unavailable: 1, pending: 1 }); }
  function applyAgentsRecentMirrorCacheControls() {}
  function classifyAgentsRecentMirrorCacheCounts() { return 'metadata'; }
  function rankAgentsRecentMirrorCacheState() { return 2; }
  function formatAgentsRecentMirrorCacheSummary() { return '1 transcript / 1 assets / 1 metadata only / 1 unavailable / 1 pending'; }
  ...
  function renderAgentsRuntimeProviderCacheSummary() { return '<div data-runtime-provider-cache-summary="pending">cache summary pending</div>'; }
  ...
  function summarizeAgentsRuntimeProviderCacheRows() { return countAgentsRuntimeProviderCacheBadges([]); }
  function countAgentsRuntimeProviderCacheBadges() { return { transcript: 1, assets: 1, metadata: 1, unavailable: 1, pending: 1 }; }
  ...
  async function copyMirrorSchedulerDiagnosticsCommand() {
    await navigator.clipboard.writeText('auracall api scheduler-diagnostics --port 18080 --provider chatgpt --runtime-profile default --completion-id acctmirror_paused');
  }
