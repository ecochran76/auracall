...
describe('api ops browser CLI helpers', () => {
  it('checks dashboard control wiring and linked status expectations', async () => {
    ...
    expect(formatApiOpsBrowserStatusCliSummary(summary)).toContain(
      'Dashboard config: page=ok identities=ok liveFollow=ok controls=ok agents=ok recentRuns=ok runtimeChat=ok runtimeProviderLinks=ok runtimeProviderDirectLinks=ok recentMirrorDetail=ok recentMirrorSummary=ok recentMirrorDirectLink=ok',
    );
    expect(formatApiOpsBrowserStatusCliSummary(summary)).toContain(
