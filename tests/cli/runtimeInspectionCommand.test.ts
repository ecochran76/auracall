...
describe('runtime inspection CLI helpers', () => {
  ...
  it('formats opt-in scheduler authority output for operators', () => {
    const rendered = formatRuntimeRunInspectionPayload({
      resolvedBy: 'run-id',
      queryId: 'runtime_cli_authority_1',
      queryRunId: 'runtime_cli_authority_1',
      matchingRuntimeRunCount: 1,
      matchingRuntimeRunIds: ['runtime_cli_authority_1'],
      taskRunSpecSummary: null,
      runtime: {
        runId: 'runtime_cli_authority_1',
        teamRunId: null,
        taskRunSpecId: null,
        sourceKind: 'direct',
        runStatus: 'running',
        updatedAt: '2026-04-21T15:00:45.000Z',
        queueProjection: {
          runId: 'runtime_cli_authority_1',
          sourceKind: 'direct',
          runStatus: 'running',
          ...
          claimState: 'held-by-lease',
          nextRunnableStepId: null,
          runningStepIds: ['runtime_cli_authority_1:step:1'],
          waitingStepIds: [],
          deferredStepIds: [],
          blockedStepIds: [],
          blockedByFailureStepIds: [],
          terminalStepIds: [],
          missingDependencyStepIds: [],
          activeLeaseId: 'lease:authority',
          activeLeaseOwnerId: 'runner:stale-owner',
          affinity: {
            status: 'not-evaluated',
            reason: null,
            requiredService: 'chatgpt',
      ...
      schedulerAuthority: {
        ...
        reason: 'active lease lease:authority is expired and owner runner:stale-owner is stale',
        ...
        candidates: [
          {
            runnerId: 'runner:alternate',
            hostId: 'host:cli',
            runnerLastHeartbeatAt: '2026-04-21T15:00:40.000Z',
            status: 'eligible',
            reason: null,
            queueState: 'runnable',
            claimState: 'claimable',
            ...
            projection: {
              ...
              queueState: 'runnable',
              claimState: 'claimable',
              nextRunnableStepId: 'runtime_cli_authority_1:step:1',
              runningStepIds: [],
              waitingStepIds: [],
              deferredStepIds: [],
              blockedStepIds: [],
    ...
    expect(rendered).toContain('Scheduler authority:');
    expect(rendered).toContain('Decision: reassignable-after-expired-lease');
    expect(rendered).toContain('Mutation allowed: no');
    expect(rendered).toContain('Selected runner: runner:alternate');
    expect(rendered).toContain('Local runner: runner:local');
    expect(rendered).toContain('Future mutation: scheduler-reassign-expired-lease');
    expect(rendered).toContain('Candidate count: 1');
    expect(rendered).toContain('Active lease owner freshness: stale');
  });
});
