  ...
  it('checks dashboard control wiring and linked status expectations', async () => {
    ...
    expect(result).toMatchObject({
      ...
      content: [
        {
          type: 'text',
          text: 'AuraCall ops browser 127.0.0.1:18080 is ok; dashboard=http://127.0.0.1:18080/ops/browser; apiService=ok; apiLogTail=ok; recentEvents=ok; recentEventFilters=ok; recentSchedulerDetail=ok; recentEventPersistence=ok; schedulerWhy=ok; preflight=ok; preflightRun=ok; preflightHistory=ok; preflightLog=ok; dashboard completion controls use /status; Live follow health: severity=paused posture=healthy state=idle active=1 paused=1 failed=0 cancelled=0 backpressure=none latestYield=none',
        },
      ],
      structuredContent: {
        host: '127.0.0.1',
